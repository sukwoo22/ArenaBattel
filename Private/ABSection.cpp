...
void AABSection::SetState(ESectionState NewState)
{
	...
	case ESectionState::BATTLE:
	{
		...
		GetWorld()->GetTimerManager().SetTimer(SpawnEnemyTimerHandle, FTimerDelegate::CreateUObject(this, &AABSection::OnEnemySpawn), EnemySpawnTime, false);
...
void AABSection::OnEnemySpawn()
{
	GetWorld()->GetTimerManager().ClearTimer(SpawnEnemyTimerHandle);
	auto KeyEnemy = GetWorld()->SpawnActor<AABCharacter>(GetActorLocation() + FVector::UpVector * 88.0f, FRotator::ZeroRotator);
	if (nullptr != KeyEnemy)
	{
		KeyEnemy->OnDestroyed.AddDynamic(this, &AABSection::OnKeyEnemyDestroyed);
	}
}

void AABSection::OnKeyEnemyDestroyed(AActor* DestroyedActor)
{
